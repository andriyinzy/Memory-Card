...
class QuestionView():
    ...
    def show(self):
        ...
        self.answer.setText(self.frm_model.question.answer)
        self.wrong_answer1.setText(self.frm_model.question.wrong_answer1)
        self.wrong_answer2.setText(self.frm_model.question.wrong_answer2)
        self.wrong_answer3.setText(self.frm_model.question.wrong_answer3)

class AnswerCheck(QuestionView):
    def __init__(self, frm_model, question, answer, wrong_answer1,
                wrong_answer2, wrong_answer, showed_answer, result):
        super().__init__(frm_model, question, answer, wrong_answer1, wrong_answer2, wrong_answer3)
        self.showed_answer = showed_answer
        self.result = result
    def check(self):
        if self.answer.is_checked():
...
class QuestionListModel(QAbstractListModel):
    ...
    def insert_rows(self, parent = QModelIndex()):
        position = len(self.form_list)
        self.beginInsertRows(parent, position, position)
        self.form_list.append(Question())
        self.endInsertRows()
        QModelIndex()
    def remove_rows(self, position, parent = QModelIndex()):
...
class QuestionEdit(QuestionView):
    ...
    def random_answer(list_model, w_question, widget_list, w_showed_answer, w_result):
        frm = list_model.random_question()
        shuffle(widget_list)
        frm_card = AnswerCheck(frm, w_question, widget_list[0], widget_list[1], widget_list[2],
                                widget_list[3], w_showed_answer, w_result)
        return frm_card
